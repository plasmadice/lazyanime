...
// ex. http://localhost:3000/api/consumet/anime/animepahe/speed-test
  ...
  try {
    ...
    if (res.results) {
      ...
      return NextResponse.json(
        {
          error: "No results from AnimePahe",
          description: `No results from AnimePahe.${
            query ? ` Query: ${query}` : ""
          }}`,
        },
        { status: 404 }
    ...
    return NextResponse.json(
      {
        error: err?.message,
        description: `Error while checking AnimePahe speed.`,
      },
      { status: 500 }
    )
  }
