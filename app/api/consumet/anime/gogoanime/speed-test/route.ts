...
// ex. http://localhost:3000/api/consumet/anime/gogoanime/speed-test
// ex. http://localhost:3000/api/consumet/anime/gogoanime/speed-test?query=naruto
  ...
  try {
    ...
    if (res.results) {
      ...
      return NextResponse.json(
        {
          error: "No results from Gogoanime",
          description: `No results from Gogoanime.${query ? ` Query: ${query}` : ""}}`,
        },
        { status: 404 }
      )
    }
    ...
    return NextResponse.json(
      {
        error: err?.message,
        description: `Error while checking GoGoAnime speed.`,
      },
      { status: 500 }
    )
  }
