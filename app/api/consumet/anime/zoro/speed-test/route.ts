...
// ex. http://localhost:3000/api/consumet/anime/zoro/speed-test
  ...
  try {
    ...
    if (res.results) {
      ...
      return NextResponse.json(
        {
          error: "No results from Zoro",
          description: `No results from Zoro.${query ? ` Query: ${query}` : ""}}`,
        },
        { status: 404 }
      )
    }
    ...
    return NextResponse.json(
      {
        error: err?.message,
        description: `Error while checking Zoro speed.`,
      },
      { status: 500 }
    )
  }
