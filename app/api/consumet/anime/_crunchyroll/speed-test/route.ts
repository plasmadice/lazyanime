...
// ex. http://localhost:3000/api/consumet/anime/crunchyroll/speed-test
export async function GET(request: Request) {
  ...
  try {
    const provider = await ANIME.Crunchyroll.create();
    ...
    if (res.results) {
      ...
      return NextResponse.json(
        {
          error: "No results from Crunchyroll",
          description: `No results from Crunchyroll.${query ? ` Query: ${query}` : ""}}`,
        },
        { status: 404 }
      )
    }
    ...
    return NextResponse.json(
      {
        error: err?.message,
        description: `Error while checking Crunchyroll speed.`,
      },
      { status: 500 }
    )
  }
