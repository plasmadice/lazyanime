...
// ex. http://localhost:3000/api/consumet/anime/animesaturn/speed-test
  ...
  try {
    ...
    if (res.results) {
      ...
      return NextResponse.json(
        {
          error: "No results from AnimeSaturn",
          description: `No results from AnimeSaturn.${
            query ? ` Query: ${query}` : ""
          }}`,
        },
        { status: 404 }
    ...
    return NextResponse.json(
      {
        error: err?.message,
        description: `Error while checking AnimeSaturn speed.`,
      },
      { status: 500 }
    )
  }
